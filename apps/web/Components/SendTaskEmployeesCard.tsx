...
import { ITask, useSocket } from '../app/custom-Hooks/SocketProvider'
...
function SendTaskEmployeesCard({user} : {user : searchedItems} ) {
  ...
  return (
    ...
    <div className=' ml-3'>
      <h2 onClick={pushToProfile} className="text-xl font-semibold">{user.name}</h2>
      <p className="text-gray-600">{user.email}</p>
     
      <Button onClick={sendTaskToUser} className=' bg-blue-500 text-white'>Send</Button>
    </div>
  </div>
  )
}
